...
class DQN:
    ...
    def Qpredict(self, x_):
        x = np.identity(self.InputSize)[x_:x_+1]  # one_hot
...
class Agent:
    ...
    def DoAction(self, act):
        if self.Env_ is not None:
            self.now_state, self.now_reward, self.Dying, self.info = self.Env_.step(self.Actions[act])
            self.PrintInfo()
    ...
    # Train Mode
    def _replay_train(self, train_batch):
        ...
        for state, action, reward, next_state, dying in train_batch:
            Q = np.array(self.MainQ.Qpredict(state))
            Q = Q.reshape(1, self.MainQ.OutputSize)
            # if terminal
            if reward == 1:
                Q[0, action] = reward
            else:
            ...
            x_stack = np.vstack([x_stack, np.identity(self.MainQ.InputSize)[state:state+1]])
        ...
        for episode in range(episode_num):
            ...
            while not self.Dying:
                if np.random.rand(1) < e:
                    ...
                    action = np.argmax(self.MainQ.Qpredict(self.now_state))

                prev_state = self.now_state
                self.now_state, self.now_reward, self.Dying, self.info = self.Env_.step(action)

                Replay_buffer.append((prev_state, action, self.now_reward, self.now_state, self.Dying))
